...
namespace {
    class CFrogCoinAddressVisitor : public boost::static_visitor<bool> {
    private:
        CFrogCoinAddress *addr;
    public:
        CFrogCoinAddressVisitor(CFrogCoinAddress *addrIn) : addr(addrIn) { }
...
bool CFrogCoinAddress::Set(const CKeyID &id) {
    SetData(Params().Base58Prefix(CChainParams::PUBKEY_ADDRESS), &id, 20);
    return true;
}

bool CFrogCoinAddress::Set(const CScriptID &id) {
    SetData(Params().Base58Prefix(CChainParams::SCRIPT_ADDRESS), &id, 20);
    return true;
}

bool CFrogCoinAddress::Set(const CTxDestination &dest) {
    return boost::apply_visitor(CFrogCoinAddressVisitor(this), dest);
}

bool CFrogCoinAddress::IsValid() const {
...
CTxDestination CFrogCoinAddress::Get() const {
    if (!IsValid())
...
bool CFrogCoinAddress::GetKeyID(CKeyID &keyID) const {
...
bool CFrogCoinAddress::IsScript() const {
    return IsValid() && vchVersion == Params().Base58Prefix(CChainParams::SCRIPT_ADDRESS);
}

void CFrogCoinSecret::SetKey(const CKey& vchSecret) {
...
CKey CFrogCoinSecret::GetKey() {
    CKey ret;
    ret.Set(&vchData[0], &vchData[32], vchData.size() > 32 && vchData[32] == 1);
    return ret;
}

bool CFrogCoinSecret::IsValid() const {
    bool fExpectedFormat = vchData.size() == 32 || (vchData.size() == 33 && vchData[32] == 1);
    bool fCorrectVersion = vchVersion == Params().Base58Prefix(CChainParams::SECRET_KEY);
    return fExpectedFormat && fCorrectVersion;
}

bool CFrogCoinSecret::SetString(const char* pszSecret) {
    return CBase58Data::SetString(pszSecret) && IsValid();
}

bool CFrogCoinSecret::SetString(const std::string& strSecret) {

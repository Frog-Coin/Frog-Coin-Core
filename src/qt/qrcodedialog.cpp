...
{
    QString ret = QString("FrogCoin:%1").arg(address);
    int paramCount = 0;
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non FROG unit input in lnReqAmount, we generate the URI with FROG as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(FrogCoinUnits::format(FrogCoinUnits::FROG, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {

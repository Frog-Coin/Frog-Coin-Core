...
{
    widget->setMaxLength(FrogCoinAddressValidator::MaxAddressLength);
    widget->setValidator(new FrogCoinAddressValidator(parent));
...
bool parseFrogCoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    // NovaCoin: check prefix
    if(uri.scheme() != QString("FrogCoin"))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if(!FrogCoinUnits::parse(FrogCoinUnits::FROG, i->second, &rv.amount))
                {
                    return false;
                }
            }
...
bool parseFrogCoinURI(QString uri, SendCoinsRecipient *out)
{
    // Convert rev:// to FrogCoin:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("rev://", Qt::CaseInsensitive))
    {
        uri.replace(0, 11, "FrogCoin:");
    }
    QUrl uriInstance(uri);
    return parseFrogCoinURI(uriInstance, out);
}
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "FrogCoin.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for FrogCoin.lnk
...
{
    return GetAutostartDir() / "FrogCoin.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        // Write a FrogCoin.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=FrogCoin\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the FrogCoin app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if(fAutoStart && !foundItem) {
        // add FrogCoin app to startup item list
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("FrogCoin-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  FrogCoin-qt [" + tr("command-line options") + "]                     " + "\n";

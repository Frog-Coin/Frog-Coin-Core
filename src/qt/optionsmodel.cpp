...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", FrogCoinUnits::FROG);
    ...
    if (!settings.contains("nAnonymizeFrogCoinAmount"))
        settings.setValue("nAnonymizeFrogCoinAmount", 1000);
    nAnonymizeFrogCoinAmount = settings.value("nAnonymizeFrogCoinAmount").toLongLong();
    if (settings.contains("nMNengineRounds"))
        SoftSetArg("-mnenginerounds", settings.value("nMNengineRounds").toString().toStdString());
    if (settings.contains("nAnonymizeFrogCoinAmount"))
        SoftSetArg("-anonymizeFrogCoinamount", settings.value("nAnonymizeFrogCoinAmount").toString().toStdString());
...
QVariant OptionsModel::data(const QModelIndex & index, int role) const
{
    if(role == Qt::EditRole)
    {
        ...
        case AnonymizeFrogCoinAmount:
            return QVariant(nAnonymizeFrogCoinAmount);
        case UseDarkTheme:
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case MNengineRounds:
            ...
            break;
        case AnonymizeFrogCoinAmount:
            nAnonymizeFrogCoinAmount = value.toInt();
            settings.setValue("nAnonymizeFrogCoinAmount", nAnonymizeFrogCoinAmount);
            emit AnonymizeFrogCoinAmountChanged(nAnonymizeFrogCoinAmount);
            break;
        default:
            break;
        }

...
class TxViewDelegate : public QAbstractItemDelegate
{
    Q_OBJECT
public:
    TxViewDelegate(): QAbstractItemDelegate(), unit(FrogCoinUnits::FROG)
    {
                      ...
                      const QModelIndex &index ) const
    {
        ...
        painter->setPen(fUseDarkTheme ? QColor(255, 255, 255) : foreground);
        QString amountText = FrogCoinUnits::formatWithUnit(unit, amount, true);
        if(!confirmed)
        {
...
void OverviewPage::setBalance(const CAmount& balance, const CAmount& stake, const CAmount& unconfirmedBalance, const CAmount& immatureBalance, const CAmount& watchOnlyBalance, const CAmount& watchOnlyStake, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    ui->labelBalance->setText(FrogCoinUnits::formatWithUnit(nDisplayUnit, balance));
    ui->labelStake->setText(FrogCoinUnits::formatWithUnit(nDisplayUnit, stake));
    ui->labelUnconfirmed->setText(FrogCoinUnits::formatWithUnit(nDisplayUnit, unconfirmedBalance));
    ui->labelImmature->setText(FrogCoinUnits::formatWithUnit(nDisplayUnit, immatureBalance));
    ui->labelTotal->setText(FrogCoinUnits::formatWithUnit(nDisplayUnit, balance + stake + unconfirmedBalance + immatureBalance));
    //ui->labelWatchAvailable->setText(FrogCoinUnits::floorWithUnit(nDisplayUnit, watchOnlyBalance));
    //ui->labelWatchStake->setText(FrogCoinUnits::floorWithUnit(nDisplayUnit, watchOnlyStake));
    //ui->labelWatchPending->setText(FrogCoinUnits::floorWithUnit(nDisplayUnit, watchUnconfBalance));
    //ui->labelWatchImmature->setText(FrogCoinUnits::floorWithUnit(nDisplayUnit, watchImmatureBalance));
    //ui->labelWatchTotal->setText(FrogCoinUnits::floorWithUnit(nDisplayUnit, watchOnlyBalance + watchOnlyStake + watchUnconfBalance + watchImmatureBalance));
...
void OverviewPage::updateWatchOnlyLabels(bool showWatchOnly)
{
    ...
    //ui->labelFROGtotalimg->setVisible(showWatchOnly);    // show watch-only balance separator line
...
void OverviewPage::setWalletModel(WalletModel *model)
{
    ...
    // update the display unit, to not use the default ("FROG")
    updateDisplayUnit();
}
...
{
    FrogCoinGUI::gotoHistoryPage_static();
}
